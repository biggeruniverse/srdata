...
class ResearchManager(glass.GlassContainer):
	logger = logging.getLogger("savage.gui.researchmanager")
	VERTICAL_PADDING = 6;
	...
	def onEvent(self, e):
		...
		elif e.eventType == "team_reset" or (e.eventType == "player_join" and e.sourceId == savage.getLocalId()): 
			for item in self.items:
				self.itemsContainer.remove(item);
				item.erase();
			del self.items[:];
	...
	def onResearchQueued(self, action):
		self.logger.debug("RESEARCH QUEUED\n");
		r = action.item;
		item = ResearchManagerItem(r, action);
		self.itemsContainer.add(item);
		self.items.append(item);

	def onResearchCancelled(self, action):
		self.logger.debug("RESEARCH CANCELLED\n");
		for i in self.items:
			if i.action == action:
				self.items.remove(i);
				self.itemsContainer.remove(i);
	...
	def onTeamChange(self):
		for item in self.items:
			...
			item.erase();
		del self.items[:];
		#now query for what's already researching

	def findResearchPosition(self, ri):
		for i,item in enumerate(self.items):
			if item.item == ri:
				return i;
...
class ResearchManagerItem(glass.GlassContainer):
	...
	def onAction(self, e):
		...
		if self.action is not None:
			gblQueue.cancelAction(self.action);
