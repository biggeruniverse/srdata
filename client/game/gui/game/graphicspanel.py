...
class GraphicsPanel(DefaultWindow):
	def __init__(self):
		DefaultWindow.__init__(self, 'Graphics Panel');
		self.first=True;
		...
		minimapOpacity = glass.GlassSlider();
		minimapOpacity.linkCvar("cl_minimap_brightness");
		self.table.addRow("Minimap Opacity", minimapOpacity);
		...
		self.scrollArea.setWidth(self.table.getWidth()+8);
