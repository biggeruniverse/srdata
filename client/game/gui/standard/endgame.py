...
outcome.setOpaque(1);
outcome.setPosition(hbreak1px + PADDING, vbreakpx + PADDING);
...
def rebuildAwards(data):
	...
	for i, item in enumerate(data.iteritems()):
		award, idString = item;
		recipients = [savage.Player(int(id)).getFormattedName() for id in idString.split(";")];
		...
		award_owner = glass.GlassLabel(recipients[0]);
		bg = glass.GlassLabel("");
		bg.setOpaque(1);
