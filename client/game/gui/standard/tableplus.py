...
class GlassTablePlus( GlassRowStack, glass.GlassTable ):
	TOP = 0;
	CENTER = 1;
	BOTTOM = 2;
	
	def __init__(self):
		GlassRowStack.__init__(self, glass.GlassTable);
	...
	def addRow( self, *inputWidgets):
		row = GlassRowStack.addRow(self, *inputWidgets);
		if self.autoAdjust:
	...
	def adjustSizeTo( self, targetWidth=None, targetHeight=None):
		...
		if sfx < 1 or sfy < 1:
			con_println("^yWarning ^wGlassTablePlus can't be compressed.\n");
