...
class HUDCrossHair (GlassTablePlus):
	barWidthPct = 0.095;
	STATUS_INACTIVE = 0;
	STATUS_ACTIVE = 1;
	alphaStep = 40;
	...
	def __init__( self ):
		...
		self.targetName = glass.GlassLabel("LOOKIHAVEARediculouslyLongName");
	...
	def reposition( self ):
		...
		self.setY( mouseY - self.getHeight() + (self.crosshair.getHeight() )//2);
	...
	def update(self):
		...
		if obj_type.isUnitType():
			...
			if isinstance( obj, savage.Player ):
